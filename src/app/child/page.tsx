...
import { getMissionsFromDB, submitMissionProofInDB } from "@/lib/missionService"; // ✅ Pakai fungsi submit foto
import { getChildProfile } from "@/lib/childService";
import { auth } from "@/lib/firebase";
import { compressImage } from "@/utils/imageCompression"; // ✅ Fungsi kompresi gambar
import { Trophy, Coins, Star, Zap, CheckCircle, Target, Timer, RefreshCw, Camera, Loader2, AlertCircle } from "lucide-react"; 

export default function ChildDashboard() {
  // xp dan level sekarang akan merefleksikan persetujuan orang tua
  ...
  const [uploadingId, setUploadingId] = useState<string | null>(null); // ✅ State loading saat upload foto
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Catatan: Sinkronisasi XP/Coins dari Cloud ke Zustand (opsional jika sudah dihandle di layout/store lain)
    } catch (error) {
  ...
  const progressPercentage = Math.min(100, Math.max(0, (xpProgress / xpRequired) * 100));

  // ✅ FUNGSI BARU: Buka Kamera, Kompres Foto, dan Kirim ke Cloud
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, missionId: string) => {
    ...
    try {
      // 1. Kompres gambar otomatis (maksimal 500KB)
      const compressedFile = await compressImage(file);
      
      // 2. Upload ke Firebase Storage & Ubah status misi
      await submitMissionProofInDB(missionId, compressedFile);
      
      alert("Bukti foto berhasil dikirim! Tunggu Ayah/Bunda memeriksa ya! 🌟");
      
      // 3. Tarik data ulang untuk merubah tombol jadi kuning (Pending)
      fetchData(); 
    } catch (error) {
            ...
            {todayMissions.map((mission) => {
              ...
              return (
                <div key={mission.id} className={`bg-white p-4 rounded-3xl border-2 shadow-sm flex items-center justify-between transition-all duration-300 ${isDone ? 'border-emerald-200 opacity-70' : isRejected ? 'border-rose-200' : 'border-slate-100'}`}>
                  ...
                  {/* ✅ Area Tombol Eksekusi / Kamera (Sudah disesuaikan) */}
                    ...
                    {isPendingApproval ? (
                      ...
                      <label className={`w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all shadow-sm cursor-pointer active:scale-90 relative ${
                        ...
                        <input 
                          type="file" 
                          accept="image/*" 
                          capture="environment" // Otomatis membuka kamera belakang di HP anak!
