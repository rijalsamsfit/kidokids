...
import { ChevronLeft, Lock, Play, Star } from "lucide-react";
...
export default function LevelMap() {
  ...
  const currentProgressLevel = 1; // Anak baru di level 1
              ...
              <div key={levelNum} className="flex flex-col items-center gap-2">
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3].map((star) => (
                    <Star 
                      key={star} 
                      className={`w-3 h-3 ${levelNum < currentProgressLevel ? "text-amber-400 fill-amber-400" : "text-white/20"}`} 
                    />
                  ))}
                </div>
              </div>
